---
title: "Station usage - hours (OBP method)"
format: html
---
...
The distribution of RSSI for our data is shown below (e.g., RSSI \~70/100/\>143 indicates the bird is likely in the far/medium/close range of the antenna's detection capability).

```{r rssi, message = FALSE, warning = FALSE, eval = TRUE, echo = TRUE, results = 'hide', include = FALSE}
...
Motus protocols group detections at a receiver into runs of detections. A run is defined as a series of detections (hit) without a gap between sequential detections of more than 60 missed detections [(Motus 2022)](https://docs.motus.org/en/tags/tag-aliasing).
...
\*NOTE: this part must be investigated in the future as it is mostly to avoid overlaps in detecting same tags from two antennas of a similar receiver\*
...
visit_duration <- runs %>%
  ...
  group_by(Band.ID, recvDeployName) %>%
  mutate(
    gapHours = as.numeric(runStart - lag(runEnd)) / 3600,
...
```{r site use filter, message = FALSE, warning = FALSE, eval = TRUE, echo = TRUE, include = FALSE}
ggplot(visit_duration %>%
         filter(duration_h > 20),
       aes(x = duration_h, y = after_stat(count))) +
...
**It remains to attribute at each visit period a tide category.**

```{r site use tide, message = FALSE, warning = FALSE, eval = TRUE, echo = TRUE, results = 'hide', include = TRUE}
# Convert to dt
tide_dt  <- as.data.table(tide_data %>% 
                            select(tideDateTimeAus, day_night, high_low, tideHeight, tideCategory))
visit_dt <- as.data.table(visit_duration)

# Sort both datetime col
setkey(tide_dt, tideDateTimeAus)
setorder(visit_dt, visitStart)

# Rolling join: for each visitStart, find the tideDateTimeAus closest in time
visit_duration <- tide_dt[visit_dt, 
                          on = .(tideDateTimeAus = visitStart), 
                          roll = "nearest"]

# Rename the matched tide time column back for clarity, and restore visitStart
setnames(visit_duration, "tideDateTimeAus", "visitStart")
...
used_bird_recv_time <- visit_duration %>%
  group_by(Band.ID, speciesEN, recvDeployName, tideCategory, day_night, high_low) %>%
...
available_bird_recv_time <- available_bird_recv_time %>%
  mutate(tideCategory = forcats::as_factor(tide_values$tideCategory))
...
```{r Available plot, message = FALSE, warning = FALSE, echo = TRUE, include = FALSE,, eval = TRUE }
ggplot() +
  geom_histogram(
...
use_rate_plot %>%
  filter(rate_use > 100) %>%
  select(Band.ID, speciesEN, recvDeployName, tideCategory, rate_use)
