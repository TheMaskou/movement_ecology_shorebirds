...
T-test looks noisy and is not powerful since it remains a non-parametric test. Trying to improve this matter we'll run a Two-way ANOVA Analysis for entropy metrics.
...
::: blockquote-yellow
**Important**  
We only keep species which holds five or more individuals tagged and detected before to run ANOVA analysis on the data.
:::

Before to run ANOVA analysis we must check the **data verify the ANOVA Assumptions.**

```{r shannon tide 521, message = FALSE, warning = FALSE, eval = TRUE,  echo = FALSE}
# Filter species with enough individuals
entropy_aov_data <- entropy_for_plot_tide %>%
  group_by(speciesEN) %>%
  filter(n_distinct(Band.ID) >= 4) %>%
  mutate(
    Band.ID     = droplevels(as.factor(Band.ID)),
    tideHighLow = droplevels(as.factor(tideHighLow))) %>%
  ungroup()

species_list_entropy <- unique(entropy_aov_data$speciesEN)
```

```{r shannon tide 52, message = FALSE, warning = FALSE, eval = TRUE,  echo = TRUE}

results      <- data.frame()
assumptions  <- data.frame()
species_list <- unique(species_list_entropy$speciesEN)


for (sp in species_list) {
  for (m in metrics_list) {
    
    df <- subset(entropy_aov_data, speciesEN == sp & metric == m)
    df <- na.omit(df)
    df$Band.ID     <- droplevels(df$Band.ID)       # drop unused factor levels
    df$tideHighLow <- droplevels(df$tideHighLow)
    
    # Skip if not enough data
    if (nrow(df) < 6 || nlevels(df$Band.ID) < 2) next
    ...
    # Checked by design: each Band.ID is a unique individual (not repeated
    # within a tide × metric cell), so observations are independent by design.
    indep_note <- "By design"
    
    # --- ASSUMPTION 2: Normality of residuals (Shapiro-Wilk) ---
    res <- residuals(model)
    sw  <- shapiro.test(res)
    ...
    assumptions <- rbind(assumptions, data.frame(
      ...
      Homogeneity      = ifelse(!is.null(lev),
                           ifelse(lev$`Pr(>F)`[1] > 0.05, "✓ Met", "✗ Violated"),
                           "Insufficient levels"),
      stringsAsFactors = FALSE))
  }
}
...
```{r shannon tide 51, message = FALSE, warning = FALSE, eval = FALSE,  echo = FALSE}

# Filter sufficient sample size
aov_data_tide <- entropy_for_plot_tide %>%
  group_by(speciesEN) %>%
  filter(n_distinct(Band.ID) >= 5) %>%
  mutate(Band.ID = as.factor(Band.ID))

# Test metric per metric
species_list <- unique(aov_data_tide$speciesEN)
metrics_list <- unique(aov_data_tide$metric)
...
for (sp in species_list) {
  ...
  for (m in metrics_list) {
    ...
    df <- subset(aov_data_tide, speciesEN == sp & metric == m)
    df <- na.omit(df)
...
Now the ANOVA assumptions verified, we can proceed the ANOVA analysis.  
  
  

```{r shannon tide 522, message = FALSE, warning = FALSE, eval = TRUE,  echo = FALSE}
...
for (sp in species_list) {
  for (m in metrics_list) {
    ...
    # Extract rows for Band.ID and tideHighLow
    for (term in c("Band.ID", "tideHighLow")) {
      if (term %in% rownames(s)) {
        results <- rbind(results, data.frame(
          Species   = sp,
...
jsd_results_tab_tide <- jsd_results_tab_tide %>%
  ...
  select(speciesEN, tideHighLow, n_indiv, n_detect, n_comparisons, mean_JSD, sd_JSD, 
...
We now want to test whether JSD values are different across individual of a same species at High and Low tides. To do so, we will conduct a two-paired ANOVA analysis, therefore we must verify data normality and homogeneity first.
  
    
::: blockquote-yellow
**Important**  
We only keep species which holds five or more individuals tagged and detected before to run ANOVA analysis on the data. Plus, we only keep individuals with more than
:::

```{r jensen-shannon tide 41, message = FALSE, warning = FALSE, eval = TRUE,  echo = FALSE}
# Filter species with enough individuals
jsd_aov_data <- jsd_for_plot_tide %>%
  group_by(speciesEN) %>%
  filter(n_distinct(Band.ID) >= 4) %>%
  mutate(
    Band.ID     = droplevels(as.factor(Band.ID)),
...
species_list_jsd <- unique(jsd_aov_data$speciesEN)
```

```{r jensen-shannon tide 42, message = FALSE, warning = FALSE, eval = TRUE,  echo = TRUE}
results_jsd     <- data.frame()
assumptions_jsd <- data.frame()

for (sp in species_list_jsd) {

  df <- subset(jsd_aov_data, speciesEN == sp)
  df <- na.omit(df)
  df$Band.ID     <- droplevels(df$Band.ID)
  df$tideHighLow <- droplevels(df$tideHighLow)
  ...
  for (term in c("Band.ID", "tideHighLow")) {
    if (term %in% rownames(s)) {
      results_jsd <- rbind(results_jsd, data.frame(
        Species = sp,
        Metric  = "JSD",
        Term    = ifelse(term == "tideHighLow", "Tide effect", term),
        Df      = s[term, "Df"],
...
```{r jensen-shannon tide 43, message = FALSE, warning = FALSE, eval = TRUE,  echo = FALSE}
results_jsd <- results_jsd %>%
  mutate(
    sig = case_when(
      p_value < 0.001 ~ "***",
      p_value < 0.01  ~ "**",
...
```{r jensen-shannon tide 45, message = FALSE, warning = FALSE, eval = TRUE,  echo = TRUE}
results_jsd <- results_jsd %>%
  mutate(
    sig = case_when(
      p_value < 0.001 ~ "***",
...
Let's now visualise the **JSD results** with a boxplots.
