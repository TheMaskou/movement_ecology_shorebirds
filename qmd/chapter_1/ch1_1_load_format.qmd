...
source(here::here("qmd", "chapter_1", "R", "ch1_1_load_format.R"))
```
...
```{r 1 packages, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}
...
-   You will have to separately run `R/1_download_motus_data.R` (or the earlier standalone script on SharePoint) before continuing. Indeed, `tagme( )` will prompt and require *username* and *password* that can't be fed here.
...
```{r 2 filter the noise plot things, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}

# Checking 'motusFiltered in' tag data
ggplot(df.new.prefilter %>%
...
```{r filter plots display, message=FALSE, warning=FALSE, eval=TRUE, echo=FALSE}
if (exists("plot_filter_in"))   plot_filter_in
if (exists("plot_filter_out"))  plot_filter_out
if (exists("plot_filter_perc")) plot_filter_perc
```
...
```{r filter recv, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}
...
```{r filter recv 2, message=FALSE, warning=FALSE, eval=FALSE, echo=TRUE}
...
# Load spreadsheet (committed CSV — download from SharePoint and update when new birds are caught)
...
# Tidal curve spline and index function
...
# Dated backups (written alongside the fixed-name files for audit trail)
