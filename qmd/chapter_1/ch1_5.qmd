...
# Source ch1_3 for codex
knitr::purl(here::here("qmd", "chapter_1", "ch1_3.qmd"), 
...
::: blockquote-blue
**Load your data** in your R environment *- see [Load & Format \> Reproducibility](qmd/ch1_5.qmd)*
:::
...
recv.act.term <- recv.act %>%
  left_join(recv %>% 
              ...
              select(deviceID, serno, stationName),
            "deviceID") %>%
  filter(!is.na(stationName)) %>%
  mutate(SernoStation = paste0(stationName, "_", serno))
...
We get `recv.act` table with every rows of activity the variables `lisStart` and `lisEnd` that specifies the complete period of time the station `stationName` has been deployed with what `deviceID`.
...
recv_hours <- recv %>%
  select(stationName, deviceID, SernoStation, lisStart, lisEnd) %>%
  rowwise() %>%
...
library(gt)
uptime_summary  %>%
  mutate(Station = recode(Station, !!!station_rename)) %>%
  gt() %>%
...
```{r 2 day MOTUS array survey effort,  message = FALSE, warning = FALSE, eval = FALSE, echo = FALSE}
...
::: panel-tabset
```{r 2 station MOTUS array survey effort plot, echo = FALSE, eval = FALSE, results = 'asis'}
