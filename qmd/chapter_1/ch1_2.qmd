...
However, it might be useful and more informative to observe fine and direct movement events from the detections, and to breakdown the data into different groups depending particular point of views and wishes.
...
table_1_pub <- table_1_pub %>%
  ...
  tab_footnote(
    footnote = md("**Legend.**Mean and standard error values are systemtically provided (x̄ \u00B1 SE, with SE = SD/\u221An). Grouped by species, this table summarises the number of shorebird individuals tagged and tracked using MOTUS-VHF technology in the Hunter estuary near Newcastle (NSW, Australia). Birds have been surveyed from the day they have been tagged (First day - at species level) to their last detection (Last day), which gives the Total of days. However, detections occurred only on certain days (Detected days - at species level). We also looked at the number of sites each species might visit per day (Nb of sites/days) and the Total number of sites the species visited during its whole survey. Dates are in dd-mm-yyyy format."))  %>%
...
```{r print table, message = FALSE, include = FALSE, warning = FALSE, echo = FALSE, eval = FALSE}
gtsave(
  data = table_1_pub,
...
And have a hint about the tidal-diel **conditions that each species is detected for** in the figure below *- see further details in [Station usage - hours \> Detection conditions](https://themaskou.github.io/movement_ecology_shorebirds/qmd/chapter_1/ch1_7.html#detection-conditions)*. <br>
...
Not displayed as it remains confidential.

<!-- ::: {.callout-tip collapse="true" title="See all detection data"} -->

<!-- ```{r 1 Detection data, message = FALSE, warning = FALSE, eval = TRUE, echo = FALSE} -->
...
See below how the tide behave in **average** during **summer**.

::: {.callout-tip collapse="true" title="See tide table"}
...
```{r 1 Tide table curve, message = FALSE, warning = FALSE, eval = TRUE, echo = FALSE}
library(dplyr)
library(ggplot2)
library(lubridate)
...
tide_hourly <- tide_data %>%
  ...
  mutate(hour = hour(tideDateTimeAus)) %>% 
