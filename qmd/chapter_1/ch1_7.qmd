...
used_bird_recv_time <- data_bird %>%
  ...
  mutate(duration_h = round(duration_sec / 3600, 0),
         ...
         tideHighLow = if_else(grepl("High", tideCategory), "High", "Low")) %>%
  select(Band.ID, speciesEN, tideCategory, tideDiel, tideHighLow, duration_h, recvDeployName)
...
::: blockquote-red
**IMPORTANT**: In a very few cases, the time available (detectability period) and the time used (detections) might be defined by a different Tide Category.\
At the current date (04 March 2026), we can't see where this discrepancy occurs neither why.It looks to happen only with Far Eastern Curlew which has been detected only a few hours after the release, in Curlew Point. Then, available time for this bird occurs only during Diurnal Low Tide while the used time for this bird falls in Diurnal High Tide. It must be unconsistency going on with the hour bin definition for the bird detection and the station listening period. However, after a full day spent on trouble shooting, the issue remain unclear and unsolved.

We decide to ignore that fact.
:::
...
::: blockquote-yellow
**Note:** We decided to force **n = `r nrow(above100)`** out of `r nrow(all)` cases where the *rate used* was between 100 and 140 % to equal 100 %.\
...
figure_plot <- figure_plot %>%
  ...
  mutate(rate_use = ifelse(rate_use > 100 & rate_use < 140, 100, rate_use)) %>% # force 100-120 rate values to equal 100 %
  mutate(rate_use = ifelse(rate_use >= 140, NA, rate_use)) %>%                  # remove rates above 120 %
...
```{r if publi Used rate X, message = FALSE,  warning = FALSE, echo = FALSE, eval = FALSE, results = 'asis'}

figure_plot <- figure_plot %>% filter(!speciesEN %in% c('Eurasian Whimbrel', 'Curlew Sandpiper'))
...
```{r figure, message = FALSE,  warning = FALSE, echo = FALSE, eval = FALSE, results = 'asis'}
make_plot <- function(species_name, recv_order, min_n_nonzero = 2) {
  ...
  # Identify recvDeployName × tideHighLow combinations that have at least some data
...
plots_used_rate[[2]] #BTG
plots_used_rate[[3]] #PS
plots_used_rate[[4]] #RNA
```
