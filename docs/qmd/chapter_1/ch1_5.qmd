...
-   `deviceID`: Raspberry Pi processor ID. A small electronic part of the receiver that plays the role of a small computer.
...
-   When a **station is listening**, the table has either values or *NA* for `pulseCount` or/and for `numTags` variables. For example:
    ...
    -   No noise has been recorded but one or several tags have been recorded: `pulseCount` = 0 or is *NA* and `numTags` \> 0

    -   No tag has been recorded but noise happened: `pulseCount` \> 0 and `numTags` = 0 or is *NA*

    -   Tag and noise have been recorded: `pulseCount` \> 0 and `numTags` \> 0

-   When a **station is** **not listening** (or there is no noise and no tags in the surrounding - assumed negligible), there is no row at all in the table (no data recorded).
...
```{r motus_survey_h, echo = FALSE,  message = FALSE, warning = FALSE, fig.width = 12, fig.height = 6, out.width = "100%"}
motus_survey_h
```
:::
...
::: column-body-outset-right
```{r motus_survey_d, echo = FALSE, message = FALSE, warning = FALSE, fig.width = 12, fig.height = 6, out.width = "100%"}
motus_survey_d
```
:::
...
## Condition of detections

Find in [Chapter 1 \> Station usage - hours \> Condition detections](https://themaskou.github.io/movement_ecology_shorebirds/qmd/chapter_1/ch1_7.html#detection-conditions) the number of hours a species have been detected and where, depending the four categories of tide (Low, High, Day, Night).
