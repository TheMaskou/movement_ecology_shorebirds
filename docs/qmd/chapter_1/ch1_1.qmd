...
Our Motus project aims to better understand movements and habitat preferences for shorebird populations within the local estuaries of the Hunter and Port Stephens.
...
`motus::dbConnect()` links your .sql to your environment to your environment as a *lazy* table, preventing the entire database from being written straight to hardware memory.

`motus::tbl()` extracts all the tags data into your environment. Then formatted into a classic dataframe.
...
::: blockquote-red
**WARNING**: Onward, values and variable names might change depending your own context.
:::
...
-   Tag's `speciesEN` information have been changed from *NA* to different values depending case to case;
...
ggplot(df.alltags %>%
  ...
  labs(x = "Motus Station", y = "Good detections (Count when motusFilter = 1)") 
...
ggplot(df.alltags %>%
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Motus Station", y = "False detections (Count when motusFilter = 0)")
...
perc <- ggplot(df.alltags %>% 
  ...
  scale_fill_manual(values = c("0" = "orange",
                               "1" = "steelblue"),
                    labels = c("False detections (filtered out)", 
                               "Good detections"),
                    name = "motusFilter") +
  theme_minimal() +
...
Day one for our local Motus array to start *listening* any tag signals is set on the **31st January 2023** - one month before the first day a bird has been caught and tagged (see [SharePoint](https://uonstaff.sharepoint.com/:x:/r/sites/StudentGroupPhD-LouiseWilliamsandMatteaTaylor/_layouts/15/Doc2.aspx?action=edit&sourcedoc=%7Ba23e66f0-c551-4ba4-acd0-de0d5e83ca42%7D&wdOrigin=TEAMS-MAGLEV.teamsSdk_ns.rwc&wdExp=TEAMS-TREATMENT&wdhostclicktime=175385) spreadsheet).

## Import band ID

As we might re-trap individuals and re-tag them with a new MOTUS tag, we can't use the Motus tag ID as unique ID anymore and have to use band number which supposes to last on the same bird from banding date to individual's death. Unfortunately, the band number is not imported by MOTUS network at the moment. So, we need to import band number (see [SharePoint](#0) spreadsheet).
...
### Signal strength, Tidal & Circadian periods
...
To reproduce any of the analysis of this Research Project, simply [**click and download the data**](https://uoneduau-my.sharepoint.com/:u:/g/personal/c3541851_uon_edu_au/IQAZKKtGel8wQofOHq3LC5D3AeBS8CjSOzqm1_ZozVyHbiU?e=5jWuMm) *(a password might be required - maxime.marini\@uon.edu.au)*.
...
-   `df.alltags`: contains all the raw detections for each Lotek nanotags

-   `df.recvDeps`: contains all the details for each receiver of the Motus array

-   `recv.act`: activity table for each receiver of the Motus array

-   `spreadsheet`: contains all the capture events details provided for each trapped individual

-   `tideData`: tidal details for Newcastle, NSW
